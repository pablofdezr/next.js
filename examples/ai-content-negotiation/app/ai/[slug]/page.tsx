...
export default async function CityPage({
  ...
  return (
    <article className="border border-slate-200 p-6 rounded-lg">
      ...
      <h1 className="text-3xl font-semibold text-slate-900 mb-4">{city.name}</h1>
      <p className="text-lg text-slate-700 mb-6">{city.summary}</p>
      <div className="prose prose-slate max-w-none text-slate-600">
        <p className="mb-4">{city.description}</p>
        <h2 className="text-xl font-semibold text-slate-900 mt-8 mb-4">Highlights</h2>
...
export async function experimentalGenerateAI(
  ...
  if (!city) {
    ...
    return {
      ...
      json: {name: 'Unknown city', slug, summary: 'Unknown city', description: 'Unknown city', population: 0, highlights: [], coordinates: {lat: 0, lng: 0}},
